...
def getAttributes(_tcp_object):
    ...
    _opt = _tcp_object.opts and "".join(
        map(binascii.hexlify, _tcp_object.opts)) or None

    return _ret % (_sport, _dport, _seq, _ack, _header_length,
                   _urg, _ack, _psh, _rst, _syn, _fin, _win, _sum, _urp, _opt)
